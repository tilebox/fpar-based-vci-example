#!/usr/bin/env python3
"""
Example of how to use the VCI GIF visualization workflow with datetime ranges.
"""

from tilebox.workflows import Client as WorkflowsClient
from vci_visualization import CreateVciGif


def create_vci_gif_example():
    """Example of creating a VCI GIF with datetime range."""
    
    # Your VCI datacube job ID
    job_id = "01986054-7c60-3d95-7ca6-e996b68b577b"
    
    # Example 1: Create GIF for a specific year
    ...
    # print(f"Submitted VCI GIF job for 2020: {job.id}")
    # print(f"Monitor at: https://app.tilebox.com/workflows/jobs/{job.id}")
    
    # Example 2: Create GIF for a specific season
    task2 = CreateVciGif(
        job_id=job_id,
    ...
    print(f"Submitted VCI GIF job for June 2021: {job2.id}")
    
    # Example 3: Create GIF for entire dataset (no time range)
    # task3 = CreateVciGif(
    #     job_id=job_id,
    ...
    # print(f"Submitted VCI GIF job for entire dataset: {job3.id}")


def create_vci_gif_drought_analysis():
    ...
    for time_range, description in drought_periods:
        ...
        print(f"Submitted {description} GIF job: {job.id}")
...
if __name__ == "__main__":
    print("VCI GIF Visualization Examples")
    print("=" * 40)
    
    print("\n1. Basic Examples:")
    create_vci_gif_example()
    
    print("\n2. Drought Analysis Examples:")
    # create_vci_gif_drought_analysis()
